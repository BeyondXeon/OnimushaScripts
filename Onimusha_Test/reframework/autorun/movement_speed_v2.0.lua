-- MoveSpeed v2.0 -- Onimusha: Way of the Sword, REFramework Lua autorun.
--
-- New mechanism, learned from FasterInteractions: each ACTION object carries
-- its own _UseOverrideMotionSpeed / _OverrideMotionSpeed fields. Setting them
-- on a locomotion action scales that action's clip AND its root motion
-- (their ladder note proves root motion follows), via plain float writes -
-- no structs, no layers, no rate vectors, no boost.
-- Applied at doEnter, every action - loops AND Start/Stop/Turn transitions -
-- runs at full mult from frame 1. No detection lag, no hold timer, no
-- suppression windows: attacks and interactions simply never get the flag
-- (AtkSpeed and FasterInteractions own those actions).
-- Retired: layer scaling, root-rate writes, displacement boost, hold latch.
--
-- Menu: REFramework -> ScriptRunner -> "MoveSpeed v2.0".
-- Config: reframework/data/movement_speed.json (same {mov} as v1.x).

local MOD, VERSION, CFG_FILE = "MoveSpeed", "2.0", "movement_speed.json"
