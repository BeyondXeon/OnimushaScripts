-- MoveSpeed v2.2 -- Onimusha: Way of the Sword, REFramework Lua autorun.
--
-- v2.2: x3 lands fully (use=true speed=3.0, 22 layers) yet travel only goes
-- 1.62 -> 2.74 m/s, so something downstream CAPS velocity. This build adds
-- an action field census: every new active action gets its numeric fields
-- dumped to the proof, hunting the cap (max speed / velocity clamp / move
-- rate on the action or its supporters). Speed logic unchanged from v2.1.
--
-- Menu: REFramework -> ScriptRunner -> "MoveSpeed v2.0".
-- Config: reframework/data/movement_speed.json (same {mov} as v1.x).

local MOD, VERSION, CFG_FILE = "MoveSpeed", "2.2", "movement_speed.json"
...
-- v2.2 census: numeric fields of the active action, hunting the velocity cap.
local census_action, census_fields = "-", {}
local function census(action, name)
    census_action, census_fields = name or "?", {}
    local ok_td, td = pcall(action.get_type_definition, action)
    if not ok_td or td == nil then return end
    local ok_f, fields = pcall(function() return td:get_fields() end)
    if not ok_f or fields == nil then return end
    local n = 0
    for _, f in pairs(fields) do
        ...
        if fname ~= nil and fname ~= "value__" then
            local ok_v, v = pcall(f.get_data, f, action)
            if ok_v and (type(v) == "number" or type(v) == "boolean") then
                local s = tostring(v)
                if #s > 24 then s = s:sub(1, 24) end
                census_fields[fname] = s
                n = n + 1
            end
        end
    end
...
local function write_proof()
    pcall(json.dump_file, PROOF_FILE, {
        ...
        census_action = census_action, census_fields = census_fields,
    })
end
...
re.on_pre_application_entry("UpdateMotion", function()
    ...
    if active ~= nil then
        ...
        meter_last = nil
        rb_use, rb_speed = nil, nil -- v2.2: no stale readback when idle
    end
end)
...
re.on_draw_ui(function()
    ...
    local census_n = 0
    for _ in pairs(census_fields) do census_n = census_n + 1 end
    imgui.text("Census: " .. census_action .. " (" .. census_n .. " fields -> proof)")
    imgui.tree_pop()
end)
