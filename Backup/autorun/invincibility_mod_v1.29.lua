-- Invincibility v1.29 -- Onimusha: Way of the Sword, REFramework Lua autorun.
--
...
--
-- Menu: REFramework -> ScriptRunner -> "Invincibility v1.29".
-- Config: reframework/data/invincibility_mod.json (stable across versions).

local MOD, VERSION, CFG_FILE = "Invincibility", "1.29", "invincibility_mod.json"
...
re.on_frame(function()
    ...
    if mode_refills() and resolve_player() then
        if hmgr ~= nil or tick % 30 == 0 then
            refill_health()
            if tick % 15 == 0 then read_hp_pair() end
        end
