-- Damage & Attack Speed v1.3 -- Onimusha: Way of the Sword, REFramework Lua autorun.
--
...
--
-- Menu: REFramework -> ScriptRunner -> "Damage & Attack Speed v1.3".
-- Config: reframework/data/damage_attack_speed.json (stable across versions).

local MOD, VERSION, CFG_FILE = "DamageSpeed", "1.3", "damage_attack_speed.json"
...
local cfg = { dmg = 1.0, atk = 1.0, act = 1.0 }
do
    ...
    if ok and type(saved) == "table" then
        ...
        if type(saved.act) == "number" then
            cfg.act = math.max(1.0, math.min(3.0, saved.act))
        elseif type(saved.spd) == "number" then
            cfg.act = math.max(1.0, math.min(3.0, saved.spd)) -- v1.0 -> v1.1 migrate
        end
    else
...
local function resolve_player()
    ...
    if player_go == nil or go:get_address() ~= player_go:get_address() then
        player_go, entity, chara, player_mod_addr = go, nil, nil, nil
    end
...
local motion, motion_go_addr, layer_orig = nil, 0, {}
    ...
    for _, pair in ipairs({ { "getLayerCount", "getLayer" }, { "getPrivateLayerCount", "getPrivateLayer" } }) do
        ...
        for i = 0, count - 1 do
            ...
            if layer ~= nil then
                ...
                if orig ~= nil and orig > 0 then
                    pcall(function() layer:call("set_Speed", orig * mult) end)
                end
            end
        end
    end
...
local function write_proof()
    ...
    pcall(json.dump_file, PROOF_FILE, {
        version = VERSION, tick = tick, cfg = { dmg = cfg.dmg, atk = cfg.atk, act = cfg.act },
...
local function action_category()
    ...
    if n:find("Run") or n:find("Walk") or n:find("Dash") or n:find("Move")
...
local function sync_root_rate(mult, want)
    ...
    if want and mult > 1.0 then
        local ok, cur = pcall(entity.call, entity, "get_ActionRootTransRate")
        if ok then
            pcall(entity.call, entity, "set_ActionRootTransRate", Vector3f.new(mult, mult, mult))
...
re.on_pre_application_entry("UpdateMotion", function()
    ...
    local want_act = cfg.act ~= nil and cfg.act > 1.0
    if not want_atk and not want_act then
    ...
    local mult = 1.0
    if cat == "attack" and want_atk then mult = cfg.atk
    elseif cat ~= "attack" and want_act then mult = cfg.act end
    if mult <= 1.0 then
        if next(layer_orig) ~= nil then restore_layers() end
        sync_root_rate(1.0, false)
        return
...
re.on_draw_ui(function()
    local ok, err = pcall(function()
        ...
        if cfg.act > 1.0 then parts[#parts + 1] = string.format("ACT x%.1f", cfg.act) end
        ...
        local chc, vc = imgui.slider_float("Action Speed", cfg.act, 1.0, 3.0, "x%.1f")
        if chc then cfg.act = vc save() end
        imgui.tree_pop()
    end)
